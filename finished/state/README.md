# React State
State describes content (text, images, etc) or visual display (dialog boxes, drop downs, colors, etc) that changes during a user's interaction with the app or website due to some event.
<br>This event could be a user's mouse click, resizing the page, navigating to a new page, etc.
<br>When state updates due to an event (a user filters a list of names for example), this content is updated across any area of the site where it is being used.
